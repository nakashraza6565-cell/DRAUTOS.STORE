...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3" style="background: #f8fafc;">
            <h6 class="m-0 font-weight-bold text-primary">Edit Product Bundle</h6>
        </div>
        <div class="card-body">
            <form method="post" action="{{route('bundles.update', $bundle->id)}}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input id="name" type="text" name="name" placeholder="e.g. Engine Maintenance Kit" value="{{$bundle->name}}" class="form-control" required>
                            @error('name')
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input id="sku" type="text" name="sku" placeholder="BNDL-001" value="{{$bundle->sku}}" class="form-control" required>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="price" class="col-form-label font-weight-bold">Bundle Price (Rs.) <span class="text-danger">*</span></label>
                            <input id="price" type="number" name="price" placeholder="Total selling price" value="{{$bundle->price}}" class="form-control" step="0.01" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input id="wholesale_price" type="number" name="wholesale_price" placeholder="0.00" value="{{$bundle->wholesale_price}}" class="form-control" step="0.01">
                        </div>
                    </div>
                     <div class="col-md-3">
                        <div class="form-group">
                            <label for="retail_price" class="col-form-label font-weight-bold">Retail Price</label>
                            <input id="retail_price" type="number" name="retail_price" placeholder="0.00" value="{{$bundle->retail_price}}" class="form-control" step="0.01">
                        </div>
                    </div>
                     <div class="col-md-3">
                        <div class="form-group">
                            <label for="walkin_price" class="col-form-label font-weight-bold">Walk-in Price</label>
                            <input id="walkin_price" type="number" name="walkin_price" placeholder="0.00" value="{{$bundle->walkin_price}}" class="form-control" step="0.01">
                        </div>
                    </div>
                     <div class="col-md-3">
                        <div class="form-group">
                            <label for="salesman_price" class="col-form-label font-weight-bold">Salesman Price</label>
                            <input id="salesman_price" type="number" name="salesman_price" placeholder="0.00" value="{{$bundle->salesman_price}}" class="form-control" step="0.01">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="2">{{$bundle->description}}</textarea>
                </div>
                                    ...
                                    <td>
                                        <select name="product_id[]" class="form-control selectpicker" data-live-search="true" required>
                                            ...
                                            @foreach($products as $product)
                                                <option value="{{$product->id}}" {{$item->product_id == $product->id ? 'selected' : ''}}>{{$product->title}} (SKU: {{$product->sku}})</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                ...
                <div class="form-group mb-0 text-right">
                    ...
                    <button class="btn btn-primary rounded-pill px-5 shadow-sm" type="submit">Update Bundle</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        // Init existing selectpickers
        // $('.selectpicker').selectpicker();

        $('#add-bundle-item').click(function() {
            // Clone the first row (or a template if we had one, but cloning first row is easier if it exists)
            // If we are editing, current rows value are set. We need to clear them.
            // Better to clone a clean template. But I'll clone the first row and reset values.
            ...
            select.val('');
            // We need to re-initialize selectpicker for the new row.
            // First destroy the old structure in the clone
