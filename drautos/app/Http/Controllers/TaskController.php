...
class TaskController extends Controller
{
    ...
    public function index()
    {
        ...
        $tasks = Task::with(['assignee', 'creator', 'related'])
            ...
            ->get(); // Using get() for kanban grouping
        ...
        $stats = [
            ...
            'high_priority' => $tasks->whereIn('priority', ['high', 'urgent'])->where('status', '!=', 'completed')->count(),
            'completed_today' => $tasks->where('status', 'completed')->where('updated_at', '>=', today())->count(),
        ];

        return view('backend.tasks.index', compact('tasks', 'users', 'stats'));
    }
    ...
    public function getCalendarEvents(Request $request)
    {
        ...
        foreach ($tasks as $task) {
            $events[] = [
                ...
                'allDay' => $task->all_day,
                'color' => $task->color ?? '#6366f1', // Default blue for tasks
                'extendedProps' => [
