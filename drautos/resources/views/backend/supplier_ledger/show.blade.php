                ...
                <div class="modal-body">
                    ...
                    <div id="payment_method_section" style="display:none; background: #f8f9fc; padding: 10px; border-radius: 5px; margin-bottom: 15px; border: 1px solid #e3e6f0;">
                        ...
                        <div class="form-group mb-0">
                            <label class="small font-weight-bold">Select Customer Cheque(s) to Transfer (Optional)</label>
                            <input type="hidden" name="payment_method" id="hidden_payment_method" value="cash">
                            <select name="payment_details[cheque_ids][]" id="customer_cheque_select" class="form-control form-control-sm selectpicker" multiple data-live-search="true" title="Choose Cheques...">
                            ...
                            <small class="text-info">Transferring a cheque will automatically mark it as Transferred in the system. Leave blank for Cash payment.</small>
                        </div>
                    </div>
...
<script>
    ...
    $('#t_category').on('change', function() {
        if ($(this).val() === 'payment') {
            ...
            $('#customer_cheque_select').selectpicker('val', ''); // Clear cheques
            $('#hidden_payment_method').val('cash');
        }
    });

    // Handle Cheque Selection to change payment method
    $('#customer_cheque_select').on('change', function() {
        if ($(this).val() && $(this).val().length > 0) {
            $('#hidden_payment_method').val('customer_cheque');
        } else {
            $('#hidden_payment_method').val('cash');
        }
    });
