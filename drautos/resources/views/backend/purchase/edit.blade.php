...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex justify-content-between align-items-center" style="background: #f8fafc;">
        ...
        <a href="{{route('purchase-orders.thermal', $purchaseOrder->id)}}" target="_blank" class="btn btn-warning btn-sm shadow-sm">
    ...
    <div class="card-body">
        ...
        <form method="post" action="{{route('purchase-orders.update', $purchaseOrder->id)}}">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input id="order_date" type="date" name="order_date" value="{{$purchaseOrder->order_date}}" class="form-control" required>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 bg-light mb-4 mt-3">
                <div class="card-body">
                    <h6 class="font-weight-bold mb-3">Manage Products in Request</h6>
                                ...
                                <tr class="item-added-row">
                                    <td class="align-middle">
                                        ...
                                        <div class="font-weight-bold text-gray-900">
                                            {{$item->product->title}}
                                            @if($item->product->brand) | {{$item->product->brand->title}} @endif
                                            | Rs. {{number_format($item->product->purchase_price, 0)}}
                                            @if($item->product->sku) ({{$item->product->sku}}) @endif
                                        </div>
                                    </td>
                                    <td class="align-middle" width="180">
                                        <div class="input-group input-group-sm">
                                            <input type="number" name="quantity[]" value="{{$item->quantity}}" class="form-control font-weight-bold text-right" min="0.1" step="any" required>
                                            <div class="input-group-append">
                                                <span class="input-group-text small">{{$item->product->unit}}</span>
                                            </div>
                                        </div>
                                    </td>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="status" class="col-form-label font-weight-bold">Order Status</label>
                        <select name="status" id="status" class="form-control font-weight-bold" required>
            ...
            <div class="form-group mb-0 text-right mt-3 pt-3 border-top">
                <a href="{{route('purchase-orders.index')}}" class="btn btn-secondary px-4 mr-2">Cancel</a>
                <button type="submit" class="btn btn-primary px-5 shadow-sm font-weight-bold" id="submit-order">UPDATE PURCHASE ORDER</button>
            </div>
        </form>
    </div>
</div>
