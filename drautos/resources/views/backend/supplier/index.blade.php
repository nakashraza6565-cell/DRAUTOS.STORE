                        ...
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2 text-center">
                                ...
                                <div class="mb-3 text-muted small">
                                    <div class="mb-1 phone-editable-container" data-id="{{$supplier->id}}" title="Double click to edit phone">
                                        <i class="fas fa-phone fa-sm mr-1 text-primary phone-icon-trigger"></i> 
                                        <span class="phone-text">{{$supplier->phone}}</span>
                                        <input type="text" class="form-control form-control-sm d-none phone-input" value="{{$supplier->phone}}" style="width: 140px; display: inline-block; height: 25px; font-size: 11px;">
                                    </div>
...
<script>
    $(document).ready(function() {
        ...
        // Inline Phone Editing Logic
        $('.phone-editable-container').on('dblclick', function() {
            let container = $(this);
            container.find('.phone-text, .phone-icon-trigger').addClass('d-none');
            container.find('.phone-input').removeClass('d-none').focus().select();
        });

        // Save on Enter, Cancel on Escape
        $('.phone-input').on('keyup', function(e) {
            let input = $(this);
            let container = input.closest('.phone-editable-container');
            let id = container.data('id');
            let oldVal = container.find('.phone-text').text();
            let newVal = input.val();

            if (e.which === 13) { // Enter key
                if (newVal === oldVal) {
                    input.addClass('d-none');
                    container.find('.phone-text, .phone-icon-trigger').removeClass('d-none');
                    return;
                }

                input.prop('disabled', true);
                $.ajax({
                    url: "/admin/suppliers/" + id + "/update-phone",
                    type: "POST",
                    data: {
                        phone: newVal,
                        _token: "{{csrf_token()}}"
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            container.find('.phone-text').text(response.phone);
                            input.val(response.phone);
                            // Update wa-btn data as well
                            $(`.wa-btn[data-id="${id}"]`).attr('data-phone', response.phone);
                        ...
                        input.prop('disabled', false).addClass('d-none');
                        container.find('.phone-text, .phone-icon-trigger').removeClass('d-none');
                    },
                    error: function() {
                        Swal.fire('Error', 'Failed to update phone number', 'error');
                        input.prop('disabled', false).val(oldVal);
                    }
                });
            } else if (e.which === 27) { // Escape key
                input.addClass('d-none').val(oldVal);
                container.find('.phone-text, .phone-icon-trigger').removeClass('d-none');
            }
        });
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).closest('.phone-editable-container').length) {
                $('.phone-input:not(.d-none)').each(function() {
                    let input = $(this);
                    let container = input.closest('.phone-editable-container');
                    let oldVal = container.find('.phone-text').text();
                    input.addClass('d-none').val(oldVal);
