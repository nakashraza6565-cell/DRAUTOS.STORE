...
@section('title', 'Add Die')

@section('main-content')
<div class="card shadow mb-4 border-0">
    <div class="card-header py-3">
        <h5 class="m-0 font-weight-bold text-primary"><i class="fas fa-tools mr-2"></i>Add Die</h5>
    </div>
    <div class="card-body">
        <form method="post" action="{{route('die-management.store')}}">
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input id="inputName" type="text" name="name" placeholder="Enter die name/code" value="{{old('name')}}" class="form-control" required>
                    @error('name')
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input id="rack_number" type="text" name="rack_number" placeholder="E.g. Rack A-3" value="{{old('rack_number')}}" class="form-control">
                </div>
                <div class="col-md-6 form-group">
                    <label for="die_type" class="col-form-label">Die Type</label>
                    <input id="die_type" type="text" name="die_type" placeholder="E.g. Molding, Casting, Press" value="{{old('die_type')}}" class="form-control">
                </div>
            </div>
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select name="maker_id" id="maker_id" class="form-control select2">
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{$supplier->id}}" {{old('maker_id') == $supplier->id ? 'selected' : ''}}>{{$supplier->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6 form-group">
                    <label for="making_cost" class="col-form-label">Making Cost (PKR)</label>
                    <input id="making_cost" type="number" step="0.01" min="0" name="making_cost" placeholder="0.00" value="{{old('making_cost', 0)}}" class="form-control">
                </div>
            </div>

            <div class="row d-none" id="financialTriggerGroup">
                <div class="col-md-6 form-group">
                    <label for="amount_paid" class="col-form-label">Amount Paid Now / Advance (PKR)</label>
                    <input id="amount_paid" type="number" step="0.01" min="0" name="amount_paid" placeholder="0.00" value="{{old('amount_paid', 0)}}" class="form-control">
                </div>
                <div class="col-md-6 form-group">
                    <label for="financial_account_id" class="col-form-label">Paid From Drawer/Account</label>
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input id="custody_of" type="text" name="custody_of" placeholder="Name of person/workshop in custody" value="{{old('custody_of')}}" class="form-control">
                </div>
                <div class="col-md-6 form-group">
                    <label for="custody_phone" class="col-form-label">Custody Contact Phone</label>
                    <input id="custody_phone" type="text" name="custody_phone" placeholder="E.g. 03001234567" value="{{old('custody_phone')}}" class="form-control">
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <select name="quality_status" class="form-control">
                        <option value="good" {{old('quality_status') == 'good' ? 'selected' : ''}}>Good (Ready)</option>
                        <option value="maintenance_required" {{old('quality_status') == 'maintenance_required' ? 'selected' : ''}}>Maintenance Required</option>
                        <option value="damaged" {{old('quality_status') == 'damaged' ? 'selected' : ''}}>Damaged (Inactive)</option>
                    </select>
                </div>
                <div class="col-md-6 form-group">
                ...
                <div class="gallery-photos-container" id="galleryContainer">
                    <div class="input-group mb-2 gallery-row">
                        ...
                        <div class="input-group-append">
                            <button type="button" class="btn btn-outline-primary choose-gallery-lfm" data-input=""><i class="fas fa-folder-open"></i></button>
            ...
            <div class="form-group mb-3 mt-4 text-right">
                ...
                <button class="btn btn-success font-weight-bold px-4" type="submit">Save Die</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        function setupGalleryLfm(row) {
            ...
            const uniqueId = 'gallery_input_' + galleryIndex;
        ...
        // Hide/Show financial group based on Maker & Cost selection
        function toggleFinancials() {
            const costVal = parseFloat($('#making_cost').val()) || 0;
            const supplierSelected = $('#maker_id').val() !== '';
            
            if (supplierSelected && costVal > 0) {
                $('#financialTriggerGroup').removeClass('d-none');
            } else {
                $('#financialTriggerGroup').addClass('d-none');
                $('#amount_paid').val(0);
                $('#financial_account_id').val('');
            }
        }

        $('#maker_id, #making_cost').on('change input', function() {
            toggleFinancials();
        });
